...
//	int2lcdyx(lc640_read_int(EE_DEBUG),2,5,0);
	...
	{
	if(ind==iSet_prl_new) bgnd_par("Введите новый пароль",sm_,sm_,sm_);
	else if(ind==iSet_prl_change) bgnd_par("Введите стар пароль ",sm_,sm_,sm_);
	else if((ind==iRBl_kb_prl)||(ind==iABl_kb_prl)) bgnd_par("Пароль разблокировки",sm_,sm_,sm_);
	else bgnd_par("  Введите  пароль   ",sm_,sm_,sm_);
